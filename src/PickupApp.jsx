...
function ParentView({ kids, activeKidId, setActiveKidId, setParentResponse }) {
  ...
  const [pendingChoice, setPendingChoice] = useState(null); // "yes" or "no" before confirm

  // Reset pending choice when switching kids
  ...
  const handleConfirm = () => {
    setParentResponse(kid.id, pendingChoice, pendingChoice === "yes" ? timeInput : "");
    setPendingChoice(null);
  };
        ...
        {kid.status && !pendingChoice ? (
          <div>
            <div style={kid.status === "yes" ? styles.confirmedYesBox : styles.confirmedNoBox}>
              {kid.status === "yes" ? (
                <><Check size={16} style={{ marginRight: 6 }} /> Pickup requested{kid.time ? ` at ${kid.time}` : ""}</>
              ) : (
                <><X size={16} style={{ marginRight: 6 }} /> No pickup today</>
              )}
            </div>
            <button style={styles.changeBtn} onClick={() => setPendingChoice(null) || setParentResponse(kid.id, null, "")}>
              Change response
            </button>
          </div>
        ) : (
          <>
            <div style={styles.yesNoRow}>
              <button
                ...
                style={{
                  ...styles.bigChoiceBtn,
                  ...(pendingChoice === "yes" ? styles.bigChoiceBtnYesActive : {}),
                }}
              >
              ...
              <button
                ...
                style={{
                  ...styles.bigChoiceBtn,
                  ...(pendingChoice === "no" ? styles.bigChoiceBtnNoActive : {}),
                }}
              >
